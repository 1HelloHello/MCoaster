...
public abstract class RegistrableItem extends Item {
    ...
    public RegistrableItem(RegistryKey<Item> registryKey, int maxStackSize) {
        super(new Item.Settings().component(DataComponentTypes.LORE,
                new LoreComponent(loadDescription("item." + Splinecart.MOD_NAME + "." + registryKey.getValue().getPath()))).registryKey(registryKey)
                .component(DataComponentTypes.MAX_STACK_SIZE, maxStackSize));
        this.identifier = registryKey.getValue().getPath();
    }
